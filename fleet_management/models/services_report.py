...
class ServicesReport(models.Model):
    _name = "services_report.fleet"
    _description = "services Analysis Report"
    _auto = False
    _rec_name = 'date'
...
class SaleOrderReportProforma(models.AbstractModel):
    _name = 'report.services_report.fleet_saleproforma'
    _description = 'Proforma Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        docs = self.env['services.fleet'].browse(docids)
        return {
            'doc_ids': docs.ids,
            'doc_model': 'services.fleet',
            'docs': docs,
            'proforma': True
        }
